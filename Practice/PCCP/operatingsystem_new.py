...
def solution(program):
    ...
    # Output
    output = [ 0 for _ in range(11) ]
    
    while True:
        if program_idx == len(program) and len(run_queue) == 0 and len(wait_queue) == 0:
            break

        # Event 1 - Program came in
        while program_idx < len(program) and program[program_idx][1] == t:
            to_run = program[program_idx]
            heapq.heappush(wait_queue, (to_run[0], to_run))
            
            program_idx += 1

            print(program_idx)

        # Event 2 - Program finished operation
        if len(run_queue) == 1 and run_time == run_queue[0][2]:
            run_queue.pop(0)
            run_time = 0

        # Event 3 - Program can be scheduled
        if len(run_queue) == 0:
            if len(wait_queue) > 0:
                score, next_run = heapq.heappop(wait_queue)
                run_queue.append(next_run)
                run_time = 0

                output[score] += t - next_run[1]
